...
def CommitTimes(repository, revision_count):
  parameters = urllib.parse.urlencode((('n', revision_count), ('format', 'JSON')))
  url = '%s/%s/+log?%s' % (_BASE_URL, urllib.parse.quote(repository), parameters)
  data = json.loads(''.join(urllib.request.urlopen(url).read().splitlines()[1:]))
