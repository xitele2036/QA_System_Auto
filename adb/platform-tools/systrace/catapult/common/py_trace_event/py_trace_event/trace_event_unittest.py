...
class TraceEventTests(unittest.TestCase):
  ...
  def testEnable(self):
    with self._test_trace():
      with open(self._log_path, 'r') as f:
        ...
        self.assertEqual(log_output['category'], 'process_argv')
        self.assertEqual(log_output['name'], 'process_argv')
        self.assertTrue(log_output['args']['argv'])
        self.assertEqual(log_output['ph'], 'M')
  ...
  def testFlushChanges(self):
    with self._test_trace():
      with open(self._log_path, 'r') as f:
        ...
        self.assertEqual(len(json.loads(f.read() + ']')), 2)
  ...
  def testTraceBegin(self):
    with self._test_trace():
      with open(self._log_path, 'r') as f:
        ...
        trace_event.trace_flush()
        log_output = json.loads(f.read() + ']')
        self.assertEqual(len(log_output), 2)
        current_entry = log_output.pop(0)
        self.assertEqual(current_entry['category'], 'process_argv')
        self.assertEqual(current_entry['name'], 'process_argv')
        self.assertTrue( current_entry['args']['argv'])
        self.assertEqual( current_entry['ph'], 'M')
        current_entry = log_output.pop(0)
        self.assertEqual(current_entry['category'], 'python')
        self.assertEqual(current_entry['name'], 'test_event')
        self.assertEqual(current_entry['args']['this'], '\'that\'')
        self.assertEqual(current_entry['ph'], 'B')
  ...
  def testTraceEnd(self):
    with self._test_trace():
      with open(self._log_path, 'r') as f:
        ...
        self.assertEqual(current_entry['category'], 'python')
        self.assertEqual(current_entry['name'], 'test_event')
        self.assertEqual(current_entry['args'], {})
        self.assertEqual(current_entry['ph'], 'E')
  ...
  def testTrace(self):
   with self._test_trace():
      ...
      with open(self._log_path, 'r') as f:
        log_output = json.loads(f.read() + ']')
        self.assertEqual(len(log_output), 3)
        current_entry = log_output.pop(0)
        self.assertEqual(current_entry['category'], 'process_argv')
        self.assertEqual(current_entry['name'], 'process_argv')
        self.assertTrue(current_entry['args']['argv'])
  ...
  def testTracedDecorator(self):
    ...
    with self._test_trace():
      ...
      with open(self._log_path, 'r') as f:
        ...
        current_entry = log_output.pop(0)
        self.assertEqual(current_entry['category'], 'python')
        self.assertEqual(current_entry['name'], '__main__.test_decorator')
        self.assertEqual(current_entry['args']['this'], '\'that\'')
        self.assertEqual(current_entry['ph'], 'B')
        current_entry = log_output.pop(0)
        self.assertEqual(current_entry['category'], 'python')
  ...
  def testClockSyncWithTs(self):
    with self._test_trace():
      with open(self._log_path, 'r') as f:
        ...
        current_entry = log_output.pop(0)
        self.assertEqual(current_entry['category'], 'python')
        self.assertEqual(current_entry['name'], 'clock_sync')
        self.assertTrue(current_entry['args']['issue_ts'])
        self.assertEqual(current_entry['ph'], 'c')
  ...
  def testTime(self):
    ...
    with self._test_trace():
      ...
      with open(self._log_path, 'r') as f:
        ...
        self.assertEqual(meta_data['category'], 'process_argv')
        self.assertEqual(meta_data['name'], 'process_argv')
        self.assertTrue(meta_data['args']['argv'])
        self.assertEqual(meta_data['ph'], 'M')
        self.assertEqual(open_data['category'], 'python')
        self.assertEqual(open_data['name'], 'test')
        self.assertEqual(open_data['ph'], 'B')
        self.assertEqual(close_data['category'], 'python')
        self.assertEqual(close_data['name'], 'test')
        self.assertEqual(close_data['ph'], 'E')
  ...
  def testNestedCalls(self):
    with self._test_trace():
      ...
      with open(self._log_path, 'r') as f:
        log_output = json.loads(f.read() + ']')
        self.assertEqual(len(log_output), 5)
        meta_data = log_output[0]
        ...
        self.assertEqual(one_open['category'], 'python')
        self.assertEqual(one_open['name'], 'one')
        self.assertEqual(one_open['ph'], 'B')
        self.assertEqual(one_close['category'], 'python')
        self.assertEqual(one_close['name'], 'one')
        self.assertEqual(one_close['ph'], 'E')

        self.assertEqual(two_open['category'], 'python')
        self.assertEqual(two_open['name'], 'two')
        self.assertEqual(two_open['ph'], 'B')
        self.assertEqual(two_close['category'], 'python')
        self.assertEqual(two_close['name'], 'two')
        self.assertEqual(two_close['ph'], 'E')
  ...
  def testMultiprocess(self):
    ...
    with self._test_trace():
      ...
      with open(self._log_path, 'r') as f:
        ...
        self.assertEqual(parent_open['category'], 'python')
        self.assertEqual(parent_open['name'], 'parent_event')
        self.assertEqual(parent_open['ph'], 'B')

        self.assertEqual(child_open['category'], 'python')
        self.assertEqual(child_open['name'], 'child_event')
        self.assertEqual(child_open['ph'], 'B')

        self.assertEqual(child_close['category'], 'python')
        self.assertEqual(child_close['name'], 'child_event')
        self.assertEqual(child_close['ph'], 'E')

        self.assertEqual(parent_close['category'], 'python')
        self.assertEqual(parent_close['name'], 'parent_event')
        self.assertEqual(parent_close['ph'], 'E')
  ...
  def testMultiprocessExceptionInChild(self):
    ...
    with self._test_trace():
      ...
      with open(self._log_path, 'r') as f:
        ...
        self.assertEqual(parent_open['category'], 'python')
        self.assertEqual(parent_open['name'], 'parent')
        self.assertEqual(parent_open['ph'], 'B')
        self.assertEqual(parent_close['category'], 'python')
        self.assertEqual(parent_close['name'], 'parent')
        self.assertEqual(parent_close['ph'], 'E')
