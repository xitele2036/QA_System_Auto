...
class FakeFSUnittest(unittest.TestCase):
  ...
  def testWalk(self):
    ...
    with fs:
      ...
      r = next(gen)
      self.assertEqual((os.path.normpath('/'), ['x'], ['a.txt']), r)

      r = next(gen)
      self.assertEqual((os.path.normpath('/x'), ['w', 'w2'], ['y.txt']), r)

      r = next(gen)
      self.assertEqual((os.path.normpath('/x/w'), [], ['z.txt']), r)

      r = next(gen)
      self.assertEqual((os.path.normpath('/x/w2'), ['w3'], []), r)

      r = next(gen)
      self.assertEqual((os.path.normpath('/x/w2/w3'), [], ['z3.txt']), r)
