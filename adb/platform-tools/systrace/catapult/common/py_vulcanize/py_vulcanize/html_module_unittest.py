...
class FakeLoader(object):
  ...
  def __init__(self, source_paths, initial_filenames_and_contents=None):
    ...
    if initial_filenames_and_contents:
      for k, v in initial_filenames_and_contents.items():
...
"""
    ...
    self.assertEqual([], metadata.dependent_module_names)
    self.assertEqual(
...
"""
    ...
    metadata = html_module.Parse(
        ...
        is_component=False,
        parser_results=parse_results)
    self.assertEqual(['a.foo'], metadata.dependent_module_names)
...
"""
    metadata = html_module.Parse(
        ...
        parser_results=parse_results)
    self.assertEqual([], metadata.dependent_module_names)
    self.assertEqual(['a.foo'], metadata.style_sheet_names)
  ...
  def testUsingAbsoluteHref(self):
    ...
    metadata = html_module.Parse(
        ...
        is_component=False,
        parser_results=parse_results)
    self.assertEqual(['foo.js'], metadata.dependent_raw_script_relative_paths)
...
""".rstrip()
      self.assertEqual(expected_js, js)
...
"""
    ...
    with fake_fs.FakeFS(file_contents):
      ...
      self.assertEqual(set(computed_deps),
